...
# Spanish version "Campo de minas", by unknown author
#
...
# 2016-06-04: Remove embedded control codes, print to a text file.
  ...
  10 LET inkColor=9: BORDER borderColor: PAPER paperColor: INK inkColor: CLS : BORDER borderColor
 ...
 106 IF ATTR (RN,A)=56 THEN GO TO 115
 110 PRINT AT RN,a;"?"
 115 LET K=RN-1+INT (RND*3)
 120 IF ATTR (K,A-1)<>56 THEN PRINT AT K,a-1;"?"
 130 BEEP .002,55
 132 PRINT AT 21,19;"          "
 ...
 402 FOR w=1 TO mines: PRINT AT INT (RND*16)+3,INT (RND*30)+1; INK paperColor;"?"
 410 BEEP .0015,35
 425 IF DAMSELS<>1 THEN NEXT W
 426 IF DAMSELS<>1 THEN GO TO 439
 430 PRINT AT 19,1;X$(W TO W+29)
 432 PRINT AT 2,1;X$(51-W TO 51-W+29)
 438 NEXT W
 439 IF lev<>1 THEN PRINT AT 15,0;: LIST
 440 PRINT AT 10,1; INK PAPER;"?";AT 11,15;"??";AT 10,30;"?"
 442 PRINT AT 21,0;"                               "
 445 IF DAMSELS=1 THEN GO TO 480
 446 IF damsels=9 THEN GO SUB 9900: GO TO 480
 448 PRINT AT 21,0; INVERSE 1; BRIGHT 1;"RESCATA A LAS POBRES DAMISELAS !": GO TO 450
 450 LET CC=INT (RND*12)+4: LET DD=INT (RND*6)+6: LET EE=INT (RND*6)+19
 452 FOR J=1 TO 7
 455 PRINT AT CC,DD;"?";AT CC,EE;"?"
 460 FOR E=1 TO 11: NEXT E
 461 BEEP .002,58
 462 PRINT AT CC,DD;"?";AT CC,EE;"?"
 ...
 483 PRINT AT 1,15;"??": PRINT AT 8,9; INK 1; BRIGHT 1; PAPER 7;"PUERTA CERRADA"
 485 FOR m=60 TO 10 STEP -2.5: FOR n=1 TO 7: NEXT n: BEEP .125,m: NEXT m
 487 PRINT AT 21,0;"                                "
 488 PRINT AT 8,9;"              "
 490 BEEP .0875,10
 491 PRINT AT 21,0;"               ?                "
 492 PRINT AT 21,31; INK paperColor; PAPER paperColor;"?"
 ...
 800 IF damsels>2 AND damsels<9 AND ti>50 THEN IF RND>.98 THEN GO TO 100
 900 GO TO 500
 998 STOP
1000 IF X=CC THEN IF Y=DD OR Y=EE THEN GO SUB 6000: RETURN
1001 IF damsels=9 THEN IF x=8 AND y=5+ss THEN GO TO 8444
1002 FOR W=20 TO 1 STEP -1: BEEP .003,0+W: PRINT AT X,Y;"?": BEEP .002,10: PRINT AT X,Y;"?": NEXT W
...
1190 INK 9
1200 PRINT AT 21,29; PAPER BORDER; INK 9;"   ";AT 21,0; PAPER BORDER; INK 9;" PUNTUACION MAX.= ";HSC;" POR ";H$
1300 FOR n=1 TO 100: NEXT n
1500 FOR N=1 TO 10000000
1510 IF SC=HSC THEN PRINT AT 21,0; PAPER borderColor; INK 9;"?";AT 21,31;"?"
1512 IF INKEY$="" THEN FOR M=1 TO 16: NEXT M
1517 IF SC=HSC THEN PRINT AT 21,0; PAPER borderColor; INK 9;"?";AT 21,31;"?"
...
2031 FOR m=1 TO 5: IF INKEY$="S" OR INKEY$="s" THEN GO TO 2033:
2032 NEXT m
...
2040 LET y$=y$(3 TO )
2045 IF INKEY$="E" OR INKEY$="e" THEN GO TO 2055
2050 NEXT t
2070 PRINT AT 21,0;"                                "
2080 RETURN
2998 STOP
...
5751 LET l$=INKEY$: IF l$="" THEN GO TO 5751
...
5760 IF CODE l$<49 OR CODE l$>57 THEN BEEP 1,-15: GO TO 5700
5770 LET ll=VAL l$
5780 IF ll>lev OR ll<>(INT ll) OR ll<1 THEN BEEP 1,-15: GO TO 5700
5784 LET sc=0
5788 LET damsels=ll
...
6010 FOR U=25 TO 50 STEP 5: PRINT AT X,Y;"?"
...
7050 PAPER 8: FOR N=3 TO 17: PRINT AT N-1,4; PAPER 8;"?";AT N-1,27;"?": PRINT AT 19-n,3;"?";AT 19-n,5;"?";AT 19-n,26;"?";AT 19-n,28;"?"
...
7110 PRINT ''" TU, ""?"", COMIENZAS EN LA PARTE  INFERIOR DE LA PANTALLA."
...
7145 PRINT AT I,L;"?": BEEP .035,N/2
7150 PRINT AT I,L;" ": NEXT N
7160 CLS
7170 PRINT AT 1,0;" MUEVETE UTILIZANDO LAS TECLAS   5,6,7 Y 8 [ VER LAS FLECHAS ]"
7180 PRINT ''" MINAS:  ""?"" SON DESAGRADABLES"''" EN LA PARTE SUPERIOR IZQUIERDA  DE LA PANTALLA SE TE INDICARA   CUANTAS SE ENCUENTRAN A TU AL-  REDEDOR."
...
7220 GO SUB 7057
7230 PRINT AT 2,0;" EL BICHO ""?"" (mina con patas)   TE PERSEGUIRA SIN DESCANSO -    PUEDE VOLVERSE MUY AGRESIVO."
...
7340 PRINT AT 6,14;"?": PRINT AT 0,0; PAPER 4; INK 9;"MINAS VECINAS 0"
7350 PRINT AT 11,0;" IMPOSIBLE? - SI TE PARECE IMPO- SIBLE NO DESESPERES. LOS MINA-  DORES, ""?"", NO SOLO PONEN MI-   NAS SINO QUE TAMBIEN LAS QUITAN (amables, No?) "
7370 PRINT ''; PAPER 1; INK 7;"                                 PULSA ""I"" PARA VOLVER A VER     LAS INSTRUCCIONES, OTRA TECLA   PARA JUGAR                     "
7390 LET s$=INKEY$: IF s$="" THEN GO TO 7390
7400 IF s$="I" OR s$="i" THEN GO TO 7000
7450 CLS
7500 RETURN
7990 GO TO 7990
7998 STOP
...
8018 PRINT AT 21,0;"                                "
8020 RETURN
8444 GO SUB 9300: LET DX=30: FOR u=.155 TO .005 STEP -.01: IF damsels<>9 OR x<>8 THEN PRINT AT 15,0;: LIST
8445 PRINT AT 0,dx; INK 0; PAPER 7;"? ": LET dx=dx-1: BEEP u,8: BEEP u,12: BEEP u,16: BORDER RND*7: NEXT u: FOR n=1 TO 16: PRINT AT 0,dx; INK 0; PAPER 7;"? ": LET dx=dx-1+(dx=0): BEEP u+.005,8: BORDER RND*7: BEEP u+.005,12: BEEP u,16: NEXT n
8446 PRINT AT 0,0; PAPER 7;" ": FOR t=0 TO 2
8448 FOR l=0 TO 7 STEP 7: INK l
8449 GO SUB 8450: GO TO 8700
8450 FOR n=1 TO 5
...
9092 BEEP .1 ,23
9095 LET s$=INKEY$: IF s$="" THEN GO TO 9095
9098 IF s$="i" OR s$="I" THEN GO SUB 7000
9100 RUN
9111 DATA "?",0,0,0,0,60,126,165,165
...
9333 RETURN
9340 NEXT n
9360 PRINT AT 11,5+ss; INK paperColor; PAPER paperColor;"?"
9400 FLASH 0
9450 PRINT AT 8,5+ss; FLASH 0;"?"
9490 RETURN
9499 STOP
9900 LET ss=INT (RND*11)+6: BEEP .3,-12
9901 PRINT AT 0,15;"??";AT 1,15;"??": BEEP .3,-12
